...
""")
    while True:
        ...
        for iteration in range(pageSelected*10,pageMax):
            print(str(iteration)+'.',theList[iteration]['quantity'],theList[iteration]['product'],'at $',theList[iteration]['price'])
...
def menuTakeItem(clientArgs):
    print(clientArgs[0])
    cart.append(catalog[int(clientArgs[0][0])])
    cart[-1]['quantity'] = int(clientArgs[0][clientArgs[2][1]])
    print(cart)
...
def menuMain(): #return to this after every cycle.
    ...
    To continue, press a command, then a comma.
    ...
    commaPos= [False,[0]]
    for here in range(0,len(clientInput)): # seperate the args from coms
        if (clientInput[here] == ',') and (commaPos[0] == False):
            commaPos[0] = True
            commaPos[1][0] = here
        elif (clientInput[here] == ',') and (commaPos[0] == True):
            commaPos[1].append(here - commaPos[1][0] - 1)
    if commaPos[0] == True:
        clientArgs = [clientInput[commaPos[1][0]+1:],commaPos[0],commaPos[1]]
    else:
        clientArgs = ['',commaPos[0],commaPos[1]] # a dict would be better.
